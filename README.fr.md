...
> prise en charge. Les preuves réelles de plateforme, Provider, Discord, réseau, permissions et
> packaging restent incomplètes. Ne la présentez pas comme publiée et ne l’utilisez pas comme plan
> de contrôle de production sans surveillance. Voir
> [État actuel du code source](#état-actuel-du-code-source).
...
2. Envoyez la demande de l’[installation recommandée](#installation-recommandée-confiez-la-à-votre-agent).
   L’Agent découvre lui-même `AGENTS.md` et `.agents/skills/opendelegate-init/SKILL.md` ; l’Owner n’a pas à
...
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Main et persistance    | CLI `opendelegate` fournie ; Control Plane composé ; contrats de stockage SQLite et PostgreSQL (la preuve PostgreSQL hébergée est actuellement limitée à la version 17) ; services durables de Task, approbation, audit, Artifact, enrôlement, Discord et canal Device ; réconciliation au démarrage qui échoue de façon sûre si le résultat d’une action interrompue est inconnu | Preuves d’installation sur hôte propre, de migration/restauration de la base, de redémarrage du service et de réconciliation complète sur chaque plateforme Main déclarée ; les autres versions majeures de PostgreSQL restent non vérifiées |
